...
int main( int argc, char *argv[] ) {
  ...
  /* Oracle files are searched in `oracle_dir` that may be set with
     ...
     ```C
     test_oracle( "Oracles can be stored on disk, check test-data/howto/test.orc",
                  "test.orc" );
     ```
     Executed with:
     ```bash
     $ ./tests/howto/01_test-examples-pass.exe --test-oracles=test-data/howto
     ```
   */
  test_oracle_start( stdout );
  use_sum( array, b );
  test_oracle_check_file( "Oracles can be stored on disk, check test-data/howto/test.orc", "test-data/howto/test.orc" );
